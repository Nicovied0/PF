...
export function postDog(payload) {
  return async function () {
    const post = await axios.post(`${link}/api/dogs`, payload);
    return post;
  };
}
...
export default function postMeli(title, unit_price) {
  return async function () {
    const post = await axios
      .post(`${link}/mercadopago`, title, unit_price)
      .then((res) => (window.location.href = res.data.init_point));
    return post;
  };
}
...
export const getUsers = () => {
  return async function (dispatch) {
    try {
      let json = await axios(`${link}/api/users`);
      return dispatch({
        type: GET_USERS,
        payload: json.data,
      });
...
export const getDogs = () => {
  return async function (dispatch) {
    try {
      let json = await axios(`${link}/api/dogs`);
      return dispatch({
        type: GET_DOGS,
        payload: json.data,
      });
